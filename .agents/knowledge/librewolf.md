# LibreWolf + PWAsForFirefox

## O que é

LibreWolf é o browser padrão. Sites que antes abriam com
...
O Chrome `--app` não existe no LibreWolf. O firefoxpwa instala o site como
PWA (runtime Firefox patched + extensão no LibreWolf) e o atalho só lança
o ID. O Chrome permanece no sistema até a migração de favoritos, mas já
não é default nem está nos binds.

## Arquivos envolvidos

- `modules/home/librewolf.nix` — `programs.librewolf` + native messaging
...
- `modules/home/session-variables.nix` — `BROWSER=librewolf`.
...
- `config/hypr/hyprland/keybinds.lua` — ALT+R LibreWolf; ALT+2/4/6
...
- `config/hypr/hyprland/windowrules/google-chrome.lua` — suppress maximize
  / sync fullscreen para `librewolf` e Chrome (enquanto o Chrome existir).
